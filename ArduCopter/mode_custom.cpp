...
bool guided_pos_terrain_alt_custom;                // true if guided_pos_target_cm.z is an alt above terrain
...
void ModeCustom::pva_control_start()
{
    ...
    guided_pos_terrain_alt_custom = false;
}
...
bool ModeCustom::set_destination(const Vector3f& destination, bool use_yaw, float yaw_cd, bool use_yaw_rate, float yaw_rate_cds, bool relative_yaw, bool terrain_alt)
{
    ...
    if (terrain_alt) {
        ...
        if (!guided_pos_terrain_alt_custom) {
    ...
    guided_pos_terrain_alt_custom = terrain_alt;
    guided_vel_target_cms.zero();
    guided_accel_target_cmss.zero();
    update_time_ms = millis();

    // log target
    //copter.Log_Write_Guided_Position_Target(guided_mode, guided_pos_target_cm.tofloat(), guided_pos_terrain_alt_custom, guided_vel_target_cms, guided_accel_target_cmss);
...
bool ModeCustom::get_wp(Location& destination) const
{
    ...
    case SubMode::Pos:
        destination = Location(guided_pos_target_cm.tofloat(), guided_pos_terrain_alt_custom ? Location::AltFrame::ABOVE_TERRAIN : Location::AltFrame::ABOVE_ORIGIN);
        return true;
    default:
        return false;
    }
...
bool ModeCustom::set_destination(const Location& dest_loc, bool use_yaw, float yaw_cd, bool use_yaw_rate, float yaw_rate_cds, bool relative_yaw)
{
    ...
    // log target
    //copter.Log_Write_Guided_Position_Target(guided_mode, Vector3f(dest_loc.lat, dest_loc.lng, dest_loc.alt), guided_pos_terrain_alt_custom, guided_vel_target_cms, guided_accel_target_cmss);
...
void ModeCustom::pos_control_run()
{
    ...
    if (guided_pos_terrain_alt_custom && !wp_nav->get_terrain_offset(terr_offset)) {
        // failure to set destination can only be because of missing terrain data
...
{
    guided_limit_custom.timeout_ms = 0;
    guided_limit_custom.alt_min_cm = 0.0f;
    guided_limit_custom.alt_max_cm = 0.0f;
    guided_limit_custom.horiz_max_cm = 0.0f;
}
...
{
    guided_limit_custom.timeout_ms = timeout_ms;
    guided_limit_custom.alt_min_cm = alt_min_cm;
    guided_limit_custom.alt_max_cm = alt_max_cm;
    guided_limit_custom.horiz_max_cm = horiz_max_cm;
}
...
void ModeCustom::limit_init_time_and_pos()
{
    ...
    guided_limit_custom.start_time = AP_HAL::millis();

    // initialise start position from current position
    guided_limit_custom.start_pos = inertial_nav.get_position_neu_cm();
}
...
bool ModeCustom::limit_check()
{
    ...
    if ((guided_limit_custom.timeout_ms > 0) && (millis() - guided_limit_custom.start_time >= guided_limit_custom.timeout_ms)) {
        return true;
    }
    ...
    if (!is_zero(guided_limit_custom.alt_min_cm) && (curr_pos.z < guided_limit_custom.alt_min_cm)) {
        return true;
    }

    // check if we have gone above max alt
    if (!is_zero(guided_limit_custom.alt_max_cm) && (curr_pos.z > guided_limit_custom.alt_max_cm)) {
        return true;
    }

    // check if we have gone beyond horizontal limit
    if (guided_limit_custom.horiz_max_cm > 0.0f) {
        const float horiz_move = get_horizontal_distance_cm(guided_limit_custom.start_pos.xy(), curr_pos.xy());
        if (horiz_move > guided_limit_custom.horiz_max_cm) {
            return true;
        }
    }
